...
const ServicesArea = (props) => {
    const featuredDataQuery = useStaticQuery(graphql`
        query infotechnoServicesQueryData {
            indexServicesJson(id: {eq: "services-why-us-contnet"}) {
              title
              subtitle
          }
    ...
    `);
    const servicesSecData = featuredDataQuery.indexServicesJson;
    const servicesData = featuredDataQuery.allItServicesJson.edges;
    const { anchorStyle } = props
    return (
        <SectionWrap id="porquenosotros">
            <Container>
                <Row>
                    <Col lg={12}>
                        <SectionTitle
                ...
                <Row>
                    <Col lg={12}>
                        <SectionBottom>
                            <Anchor {...anchorStyle} path="#contacto">Sueña en grande, se diferente, acepta el reto!<IoIosArrowRoundForward className="ml-1" /></Anchor>
                        </SectionBottom>
                    </Col>
                </Row>
            </Container>
